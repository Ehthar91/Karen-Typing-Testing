...
function chooseMoveDestination(event,reservation){
  ...
  return new Promise(resolve=>{
    ...
    for(const choice of choices){const button=document.createElement('button');button.type='button';button.className='event-move-option';const time=document.createElement('strong');time.textContent=formatScheduleTime(choice.slot.time);const copy=document.createElement('span');const label=document.createElement('b');label.textContent=choice.slot.label||`${choice.slot.duration||15} minute appointment`;const left=document.createElement('small');left.textContent=`${choice.spotsLeft} spot${choice.spotsLeft===1?'':'s'} available`;copy.append(label,left);button.append(time,copy);button.addEventListener('click',()=>{dialog.dataset.selectedSlotId=choice.slot.id;dialog.close('move')});options.append(button)}
...
async function moveManagedReservation(eventId,reservation,source='owner'){
  ...
  try{await initRaceFirebase();const latestSnap=await fb.get(fb.ref(db,`eventSignups/${eventId}`));if(!latestSnap.exists())throw new Error('This event no longer exists.');const latest=latestSnap.val();const latestDestination=eventSlotsArray(latest).find(slot=>slot.id===destinationId);if(!latestDestination)throw new Error('That time slot is no longer available.');newSpotId=await claimManagedMoveSpot(eventId,latest,latestDestination,manageToken);
    ...
    const message=`${student} moved to ${formatScheduleTime(latestDestination.time)}.`;if(source==='editor')sharedEditorMessage(message,'ok');else showToast(message);
...
function renderPublicEvent(){
  const event=publicEventData;if(!event)return;initializePublicSelection();eventPublicTitle.textContent=event.title||'Event Sign Up';eventPublicDate.textContent=eventDateText(event.date);eventPublicDescription.textContent=event.description||'Choose an available time slot below.';
